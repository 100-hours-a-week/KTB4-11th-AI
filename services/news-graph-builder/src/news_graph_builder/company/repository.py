...
def aliased_plain_entities(conn: sa.Connection) -> list[tuple[int, str]]:
    query = (
...
def merge_entity(conn: sa.Connection, entity_id: int, into_id: int) -> None:
    for column in (relations.c.source_entity_id, relations.c.target_entity_id):
        conn.execute(sa.update(relations).where(column == entity_id).values({column: into_id}))
    conn.execute(
        insert(cluster_entities)
        .from_select(
            ["cluster_id", "entity_id"],
            sa.select(cluster_entities.c.cluster_id, sa.literal(into_id, sa.BigInteger)).where(
                cluster_entities.c.entity_id == entity_id
            ),
        )
        .on_conflict_do_nothing()
    )
