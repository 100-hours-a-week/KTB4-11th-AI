...
| `.github/workflows/ci.yaml:37,55,68` | Add `market-collector` to the three service loops |
...
- Modify: `.github/workflows/ci.yaml` lines 37, 55 and 68

**Interfaces:**
- Consumes: the `market-collector` console script from Task 1.
- Produces: nothing importable. CI gains a fourth service in three loops.
...
`ta-lib` needs the native TA-Lib C library. If the build fails while compiling it, add the system dependency to the builder stage, immediately after `WORKDIR /app`:
...
The runtime stage then needs the shared library too, so copy it after the venv:

```dockerfile
COPY --from=builder /usr/lib/libta_lib.so* /usr/lib/
```

Check `docker/portfolio-builder.Dockerfile` first — it already depends on `ktb-market-analyzer`, so whatever it does about TA-Lib is the established answer and should be copied rather than reinvented.
...
Expected: one JSON log line with `"message":"market-collector started"` and exit code 0. This is what keeps the service compatible with the existing CI job; a bare invocation must never dial anything.

- [ ] **Step 5: Add the service to the three CI loops**

In `.github/workflows/ci.yaml`, add `market-collector` to each of the three lists:

```yaml
          for pkg in news-preprocessor news-clusterer portfolio-builder market-collector; do
```

That is line 37 (`verify-dependency-isolation`), line 55 (`verify-exported-requirements`) and line 68 (`build-and-run-images`, where the variable is `svc`).

Then add the no-network run to the `One-shot services exit 0 with networking disabled` step, after the `portfolio-builder` block:

```yaml
          docker run --rm --network none \
            -e MARKET_COLLECTOR_QUESTDB_DSN="$NEVER_DIALED_QUESTDB_DSN" \
            -e MARKET_COLLECTOR_QUESTDB_ILP_HOST="unused.invalid" \
            -e MARKET_COLLECTOR_KIWOOM_ACCOUNTS='[{"app_key":"x","secret_key":"y"}]' \
            ktb-market-collector
```

Also update the step name `Build all three images` to `Build every service image`, since it no longer builds three.

- [ ] **Step 6: Reproduce the isolation check locally**
...
Expected: no output, exit 0. A failure here means a dependency is used but not declared in `services/market-collector/pyproject.toml`.

- [ ] **Step 7: Reproduce the requirements check locally**
...
Expected: no diff. A diff means the committed export is stale.
...
```bash
git add docker .github/workflows/ci.yaml uv.lock
git commit -m "ci: build and verify the market-collector image"
```
